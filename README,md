# <h1 align = "center"> Lista de Exercícios: Python - Treinamento com Funções </h1>

<div align = "center">

<img src = ".\imgs\imagem-README.png" alt = "Imagem mostrando o Código do Exercício Nº8" heigh = "400">

</div>

### Status da Lista: Concluída ✔️ <br>

## 🗂 Sumário

- [📝 Descrição da Lista](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Lacos_de_Repeticao_for_e_while__Projeto_N2_2026#-descri%C3%A7%C3%A3o-da-lista)
- [🎯 Objetivos do Treinamento](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Lacos_de_Repeticao_for_e_while__Projeto_N2_2026#-objetivos-da-lista)
- [💻 Tecnologias Utilizadas](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Lacos_de_Repeticao_for_e_while__Projeto_N2_2026#-tecnologias-utilizadas)
- [🧩 Exercícios](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Lacos_de_Repeticao_for_e_while__Projeto_N2_2026#-exerc%C3%ADcios)
- [📂 Estrutura da Lista](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Lacos_de_Repeticao_for_e_while__Projeto_N2_2026#-estrutura-da-lista)
- [✨Autor](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Lacos_de_Repeticao_for_e_while__Projeto_N2_2026#autor)
- [👨🏻‍💻 Contatos](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Lacos_de_Repeticao_for_e_while__Projeto_N2_2026#%E2%80%8D-contatos--refer%C3%AAncias)

## 📝 Descrição da Lista

Todo programa é composto por várias partes pequenas que, juntas, formam uma solução tecnológica capaz de resolver determinados problemas, atender certas necessidades ou criar novas oportunidades. Esses pequenos blocos de código são chamados de **funções**. Elas foram criadas para vários objetivos, entre eles: modularizar o código, reutilizar comportamentos e evitar a repetição de trechos, tornando os programas mais organizados e fáceis de se manter. Por isso, dominar o uso delas não é apenas uma oportunidade, mas uma necessidade. <br>

Entre as diversas formas de aprendizagem, a prática constante está entre as mais eficiêntes para consolidar conhecimentos e é buscando aprofundar e treinar o conhecimento de funções que este repositório foi criado. Esta lista de exercícios contém, ao todo, 10 (dez) problemas baseados em situações do nosso cotidiano. Cada exercício propõe utilizar diferentes conceitos relacionados às funções dentro da linguagem de programação *Python*, tais como *lambdas*, *closures*, funções embutidas (*built-in*) e até recursividade. <br>

> [!NOTE]
> Todos os 10 (dez) casos e os objetivos de aprendizagem foram criados pela *Alura* no curso *Praticando Python: Funções* (Link disponível na seção de contatos), enquanto as resoluções apresentadas, e a redação e organização do README são de minha autoria, compartilhadas para fins acadêmicos e demonstração profissional. <br> <br>

## 🎯 Objetivos do Treinamento

- Entenda o que são funções e como utilizá-las;
- Aprenda a criar funções personalizadas em Python;
- Explore funções embutidas, recursivas, lambdas e closures.

## 💻 Tecnologias Utilizadas

![Python](https://img.shields.io/badge/Python-0?style=for-the-badge&logo=python&logoColor=%23FFFFFF&color=%233776AB)
![Git](https://img.shields.io/badge/Git-0?style=for-the-badge&logo=git&logoColor=%23FFFFFF&color=%23F05032)
![GitHub](https://img.shields.io/badge/GitHub-0?style=for-the-badge&logo=github&logoColor=%23FFFFFF&color=%23181717) <br>

## 🧩 Exercícios

**1. Calculando a idade 👴🏻;**\
**2. Contador de caracteres 🔠;**\
**3. Saudação personalizada 👋🏻;**\
**4. Conversor de tipos 🔀;**\
**5. Calculando o total de vendas 💵;**\
**6. Filtrando números pares 🔢;**\
**7. Juntando listas de produtos 🛒;**\
**8. Calculadora com lambda 🧮;**\
**9. Gerador de funções personalizadas 🔨;**\
**10. Somando números recursivamente ➕.**

## 📂 Estrutura da Lista

```
/Exercicios
    Exercicio-1.py
    Exercicio-2.py
    Exercicio-3.py
    Exercicio-4.py
    Exercicio-5.py
    Exercicio-6.py
    Exercicio-7.py
    Exercicio-8.py
    Exercicio-9.py
    Exercicio-10.py

/img
    ~imagem-README.png

README.md
```

## ✨Autor

<img src = "https://avatars.githubusercontent.com/u/187053289?v=4" width = 120px> <br>
<strong> Victor </strong>

## 👨🏻‍💻 Contatos & Referências

<a href = "https://www.linkedin.com/in/victor-m-rodrigues/"><img alt="Static Badge" src="https://img.shields.io/badge/LinkedIn-0?style=for-the-badge&logoColor=%23FFFFFF&color=%230077B5&link=https%3A%2F%2Fwww.linkedin.com%2Fin%2Fvictor-m-rodrigues%2F"></a> 
<a href = "https://github.com/Victor-M-S-Rodrigues07/"><img alt="Static Badge" src="https://img.shields.io/badge/GitHub-0?style=for-the-badge&logo=github&logoColor=%23ffffff&labelColor=%23000000&color=%2300000000"></a>
<a href = "https://cursos.alura.com.br/user/victorvicmr"><img alt="Static Badge" src="https://img.shields.io/badge/Alura-0?style=for-the-badge&color=%23100D36"></a>
<a href = "https://cursos.alura.com.br/course/praticando-python-funcoes"> <img alt="Static Badge" src="https://img.shields.io/badge/Praticando%20Python%3A%20Fun%C3%A7%C3%B5es-0?style=for-the-badge&label=Curso&labelColor=444444&color=F29999"> </a>

🇧🇷 - 2026
